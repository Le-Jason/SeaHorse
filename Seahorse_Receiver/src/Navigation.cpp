...
Navigation::Navigation()
{
    myIMU.begin();
    temp = myIMU.getTemp();
    imu::Vector<3> accel = myIMU.getVector(Adafruit_BNO055::VECTOR_ACCELEROMETER);
...
void Navigation::measurementIMU_BNO()
{
    ...
    vectorIMU.x = (accel.x() * diffTime * diffTime) + (vectorIMU.u*diffTime) +  vectorIMU.x;
    vectorIMU.y = (accel.y() * diffTime * diffTime) + (vectorIMU.v*diffTime) +  vectorIMU.y;
    vectorIMU.z = (accel.z() * diffTime * diffTime) + (vectorIMU.w*diffTime) +  vectorIMU.z;
    vectorIMU.phi = euler.roll;
    vectorIMU.theta = euler.pitch;
    vectorIMU.psi = euler.yaw;
    vectorIMU.u = (accel.x() * diffTime) + vectorIMU.u;
    vectorIMU.v = (accel.y() * diffTime) + vectorIMU.y;
    vectorIMU.w = (accel.x() * diffTime) + vectorIMU.z;
